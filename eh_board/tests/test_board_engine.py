...
class TestBoardEngine(TransactionCase):
    ...
    def test_predefined_packs_seeded_and_gated(self):
        ...
        # Vertical packs' base apps are absent in the clean-room, so each is
        # unavailable and applying one raises a friendly error.
        vertical = packs.filtered(lambda p: p.required_module)
        self.assertTrue(vertical and all(not p.is_available() for p in vertical))
        from odoo.exceptions import UserError
        with self.assertRaises(UserError):
            vertical[0].create_from_template()
